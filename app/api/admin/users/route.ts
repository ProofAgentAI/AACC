...
async function requireUser(request: NextRequest, needAdmin: boolean) {
  ...
  if (needAdmin && roleOf(data.user) !== "admin") {
    return {
      error: NextResponse.json(
...
export async function POST(request: NextRequest) {
  ...
  const role = ["admin", "board", "staff"].includes(body?.role) ? body.role : "board";
  ...
  // password, landing on /admin/setup.
...
export async function PATCH(request: NextRequest) {
  ...
  const role = ["admin", "board", "staff"].includes(body?.role) ? body.role : "";
  if (!id || !role) {
